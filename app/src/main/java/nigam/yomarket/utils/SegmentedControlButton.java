...
public class SegmentedControlButton extends android.support.v7.widget.AppCompatRadioButton {
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (isChecked()) {
            GradientDrawable grad = new GradientDrawable(Orientation.TOP_BOTTOM, new int[] { 0xffd42121, 0xffd42121 });

            grad.setBounds(0, 0, this.getWidth(), this.getHeight());
            grad.draw(canvas);
            textPaint.setColor(Color.WHITE);
        } else {
            GradientDrawable grad = new GradientDrawable(Orientation.TOP_BOTTOM, new int[] { 0xFFEB5151, 0xFFEB5151 });
            grad.setBounds(0, 0, this.getWidth(), this.getHeight());
            grad.draw(canvas);
